...
import com.bawei.demo.shoppingtrolley.evaluate.EvaluateActivity;
import com.bawei.demo.shoppingtrolley.evaluate.EvaluateGoodsAdapter;
import com.bawei.demo.shoppingtrolley.payment.PayingActivity;
...
public class OrderAdapter extends RecyclerView.Adapter<OrderAdapter.ViewHolder> {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        View view= LayoutInflater.from(context).inflate(R.layout.layout_order_code,viewGroup,false);
     ...
     @Override
        ...
        LinearLayoutManager manager=new LinearLayoutManager(context);
         ...
         switch (orderStatus){
             case 1:
                 viewHolder.textView_go_to_pay_order.setText("去支付");

                 break;
             case 2:
                 viewHolder.textView_go_to_pay_order.setText("确定收货");
                 break;
             case 3:
                 viewHolder.textView_go_to_pay_order.setText("去评价");
                 break;
             case 9:
                 viewHolder.textView_go_to_pay_order.setText("完成");
                 break;
             default:
                 break;
         }
        ...
        viewHolder.textView_go_to_pay_order.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
             if(viewHolder.textView_go_to_pay_order.getText().toString().equals("去支付")){
                 Intent intent=new Intent(context,PayingActivity.class);
                 intent.putExtra("id",list.get(i).getOrderId());
                 intent.putExtra("price", totalPrice +"");
                 context.startActivity(intent);
             }else if(viewHolder.textView_go_to_pay_order.getText().toString().equals("确定收货")){
                if(callBack!=null){
                    callBack.getOrderId(list.get(i).getOrderId());
                }

             }else if(viewHolder.textView_go_to_pay_order.getText().toString().equals("去评价")){

                 OrderBean.OrderListBean.DetailListBean bean = list.get(i).getDetailList().get(0);
                 Intent intent=new Intent(context,EvaluateActivity.class);
                 intent.putExtra("aList", (Serializable) bean);
                 intent.putExtra("orderId",list.get(i).getOrderId());
                 context.startActivity(intent);

             }else if(viewHolder.textView_go_to_pay_order.getText().toString().equals("完成")){
